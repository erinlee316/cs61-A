...
def floor_div(args):
    ...
    # set remaining values in pair
    result = calc_eval(args.first)
    remaining = args.rest

    # keep looping until 
    while remaining is not nil:
        result = result // calc_eval(remaining.first)
...
def eval_and(expressions):
    ...
    # move through all pairs
    while current is not nil:
        value = calc_eval(current.first)    
        ...
        # shoudl return the true value
        # reassign current value to next value in pair
        result = value
        current = current.rest
    
    # returns true since pair didn't result in any false values
    return result
...
def eval_define(expressions):
    ...
    # add key value pair into bindings dictionary
