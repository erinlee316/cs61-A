def divide(quotients, divisors):
    ...
    """
    # return every item in quotient as key of dictionary
    # items inside keys ONLY if divisors/quotient has no remainder
...
def buy(fruits_to_buy, prices, total_amount):
    ...
    def add(fruits, amount, cart):
        if fruits == [] and amount == 0:
            ...
            fruit = fruits[0]
            price = prices[fruit]
            # range starts at 1 --> need to buy AT LEAST one fruit
            # need (amount // price) + 1 --> inclusive of endpoint
...
def distance(city_a, city_b):
    ...
    a_latitude = get_lat(city_a)
    b_latitude = get_lat(city_b)

    a_longitude = get_lon(city_a)
    b_longitude = get_lon(city_b)

    return sqrt((b_latitude - a_latitude)**2 + (b_longitude - a_longitude)**2)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    def dist(city, lat, lon):
        ...
        return sqrt((lat - latitude)**2 + (lon - longitude)**2)
    
    a_distance = dist(city_a, lat, lon)
    b_distance = dist(city_b, lat, lon)
