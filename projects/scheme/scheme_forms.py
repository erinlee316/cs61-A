...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        # get second operand and evaluate it
        second_operand = expressions.rest.first
        evaluated_value = scheme_eval(second_operand, env)

        # bind first operand to evaluate value
        env.define(signature, evaluated_value)
        return signature
        ...
        symbol = signature.first # get symbol inside first pair
        formals = signature.rest # get remaining parameters in first pair
        body = expressions.rest # get entire body

        # create procedure
        lbd = do_lambda_form(Pair(formals, body), env)

        # bind procedure to new lbd instance
        env.define(symbol, lbd)

        # return symbol that was bound
        return symbol
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    # keep iterating through entire list
    while expressions is not nil:
        last_val = scheme_eval(expressions.first, env)

        # check if current expression is not true
        if not is_scheme_true(last_val):
            return last_val

        # move onto next expression
        expressions = expressions.rest

        # expression is false, return false
        #else: 
         #   return False

    # all expressions in list were true, return last value
    return last_val
    # END PROBLEM 12
...
def do_or_form(expressions, env):
    ...
    # keep iterating through entire list
    while expressions is not nil:
        # return true if current expression is true
        if is_scheme_true(scheme_eval(expressions.first, env)):
            return scheme_eval(expressions.first, env)

        # move onto next expression, current on is false
        else: 
            expressions = expressions.rest
    
    # no expressions in list were true, return false
    return False
    # END PROBLEM 12
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            # BEGIN PROBLEM 13
            "*** YOUR CODE HERE ***"
            # check if true predicate has sub-expressions
            # return result from function that evaluates them all
            if clause.rest is not nil:
                return eval_all(clause.rest, env)

            # no sub-expressions, return predicate
            else:
                return test
            # END PROBLEM 13
        expressions = expressions.rest
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        # get correspondings parts from current pair
        symbol = binding.first
        expression = binding.rest.first

        names = Pair(symbol, names)
        vals = Pair(scheme_eval(expression, env), vals)

        # move onto next pair
        bindings = bindings.rest

    # check to see that each symbol in list is distinct
