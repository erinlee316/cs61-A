...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            # check if existing ant can contain new ant
            if place.ant.can_contain(self):
                place.ant.store_ant(self)

            # check if new ant can contain existing ant
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        # intialize variable to keep track of each place ant goes
        distance = 0
        current_position = self.place

        # keep looping if there are no bees at current position
        while current_position is not None:
            # check if current position is hive
            if current_position.is_hive is True:
                return None
            # if current position contains bees, return a random bee
            if current_position.bees:
                if self.lower_bound <= distance and self.upper_bound >= distance:
                    return random_bee(current_position.bees) 
            
            # check next position
            current_position = current_position.entrance
            distance += 1
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        # reduce health of bees in ant's place when ant receives damage
        for bee in list(self.place.bees):
            bee.reduce_health(amount)

        # check if ant is dead
        # inflict additional damage to bees in its place if ant is dead
...
class WallAnt(Ant):
    ...
    # automatically resets to default health
...
class HungryAnt(Ant):
    ...
    # automatically resets to default cooldown and health
    def __init__(self, health=1):
        self.cooldown = 0
        super().__init__(health)

    # hungry ant's actions during his turn
    def action(self, gamestate):
        # check if ant is chewing
        # decrement number of turns left to chew
        if self.cooldown != 0:
            self.cooldown -= 1

        # ant is not chewing
        else:
            # get copy of all bees
            all_bees = self.place.bees
            
            # check list to see if there are any bees to eat 
            # select random bee that will get eaten
            if all_bees:
                chosen_bee = random_bee(all_bees)
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        # check if container ant does not have ant inside
        if self.ant_contained is None:
            # check if other ant is not container
    ...
    def store_ant(self, ant):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        # set ant to instance attribute, knows that there is an ant contained
    ...
    def action(self, gamestate):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        # check if ant is contained by container ant
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        # do damage to all bees in its place
        for bee in list(self.place.bees):
            bee.reduce_health(self.damage)

        # check if ant exists to allow it to perform its action
        if self.ant_contained:
            self.ant_contained.action(gamestate)
# END Problem 9
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        # check if insect is waterproof or not
        # reduct insect's health to 0 if not waterproof
...
class QueenAnt(ScubaThrower):
    ...
    def __init__(self, health=1):
        super().__init__(health)
        # check if queen does not exists
        # make a queen
        if QueenAnt.queen_exists is not True:
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        # queen ant does action
        super().action(gamestate)

        # initialize variable to keep track of queen's position        
        current_position = self.place.exit

        # keep looping if there are no bees at current position
        while current_position is not None:
            ant = current_position.ant
            # check if current position has ant
            # double ant's damage
            # keep track that ant was doubled
            if ant is not None:
                if ant.has_doubled is not True:
                    ant.double()
                    ant.has_doubled = True
                
                # check that ant is a container
                # double ants' inside container's damage
                # keep track that ant was doubled
                if hasattr(ant, 'is_container'): 
                    if ant.is_container and ant.ant_contained is not None:
                        if ant.ant_contained.has_doubled is not True:
                            ant.ant_contained.double()
                            ant.ant_contained.has_doubled = True
            
            # check next position
            current_position = current_position.exit
        # END Problem 12
